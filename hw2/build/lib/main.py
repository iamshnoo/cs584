...
if __name__ == "__main__":
    ...
    X_res, y_res =SVMSMOTE(random_state=42).fit_resample(train_dataset.X, train_dataset.y)
    ...
    forest = BalancedRandomForestClassifier(random_state=42, n_estimators=10000, n_jobs=-1)
    ...
    indices = [int(elem.split(" ")[1]) for elem in forest_importances.nlargest(750).index]
    train_dataset.X = train_dataset.X[:, indices]
    print(f"X shape after BalancedRandomForest feature selection: {train_dataset.X.shape}")
    ...
    clf3 = DecisionTreeClassifier(criterion="gini", max_depth=None, min_samples_split=4, splitter="best")

    ensemble = VotingClassifier(estimators=[('p', clf1), ('m', clf2), ('d', clf3)], voting='hard')
    ...
    y_pred = np.array([1 if y_pred_1[i] == 1 and y_pred_2[i] == 1 else 0 for i in range(len(y_pred_1))])
