...
from vectorize import reduce_dimensionality, train_validation_split, vectorize
...
# my implementation of KNN (supports weighted and unweighted))
class KNNClassifier:
    ...
    def _k_neighbors(self, pairwise_dist, calculate_distance=False):
        ...
        if DEBUG:
            for i in range(pairwise_dist.shape[0]):
                for j in range(pairwise_dist.shape[1]):
                    print("{:.5f}".format(pairwise_dist[i, j]), end=" ")
                print()
            print("-" * 80)
            sorted_pairwise_dist = np.sort(pairwise_dist, axis=1)
            for i in range(sorted_pairwise_dist.shape[0]):
                for j in range(sorted_pairwise_dist.shape[1]):
    ...
    def predict(self, X_test):
        ...
        if not self.weighted:
            k_nearest_neighbors_indices, _ = self._k_neighbors(
                pairwise_dist, False
            )
        else:
            (
                k_nearest_neighbors_indices,
                k_nearest_neighbors_distances,
            ) = self._k_neighbors(pairwise_dist, True)
        ...
        if not self.weighted:
            ...
            mode, _ = weighted_mode(
                y_[k_nearest_neighbors_indices, 0],
                k_nearest_neighbors_distances,
                axis=1,
            )
            mode = mode.ravel()
...
if __name__ == "__main__":
    ...
    knn = KNNClassifier(n_neighbors=5, metric="euclidean", weighted=True)
